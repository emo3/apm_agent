...
Place a dependency on the `apm_agent`` cookbook in your cookbook's
`metadata.rb`.
...
```
include_recipe 'apm_agent`::install_agent-os'
```
...
* `default['apm_agent`']['my_attribute']`: Describe the purpose or usage of
...
apm_agent`_resource_name 'some value' do
